...
// dst and isrc are expected to be different registers.
// Clobbers: rflags
void C2_MacroAssembler::neon_minmax_reduction_int(Register dst, BasicType bt, Register isrc,
                                                  FloatRegister vsrc, int vector_length_in_bytes,
                                                  bool is_min, FloatRegister vtmp) {
  assert(vector_length_in_bytes == 8 || vector_length_in_bytes == 16, "unsupported");
  assert(bt == T_BYTE || bt == T_SHORT || bt == T_INT, "unsupported");
  assert_different_registers(dst, isrc);

  SIMD_Arrangement size = esize2arrangement((unsigned)type2aelembytes(bt),
                                             vector_length_in_bytes == 16);
  BLOCK_COMMENT("neon_minmax_reduction_int {");
    if (size == T2S) {
      assert(bt == T_INT && vector_length_in_bytes == 8, "impossible");
...
// The input vector length is expected to be 16 bytes.
// Clobbers: rscratch1, rflags
void C2_MacroAssembler::neon_minmax_reduction_long(Register dst, Register isrc,
                                                   FloatRegister vsrc, bool is_min) {
  assert_different_registers(dst, isrc);
  BLOCK_COMMENT("neon_minmax_reduction_long {");
    umov(rscratch1, vsrc, D, 0);
    cmp(isrc, rscratch1);
...
void C2_MacroAssembler::sve_extract_integral(Register dst, SIMD_RegVariant size, FloatRegister src, int idx,
  ...
  if ((size == B && idx < 16) || (size == H && idx < 8) ||
      (size == S && idx < 4) || (size == D && idx < 2)) {
