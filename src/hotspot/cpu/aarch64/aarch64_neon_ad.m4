...
dnl
define(`REDUCE_ADD', `
instruct reduce_addI_$1bits`'(iRegINoSp dst, iRegIorL2I isrc, vec$2 vsrc, vec$2 vtmp) %{
...
instruct reduce_addL_128bits(iRegLNoSp dst, iRegL isrc, vecX vsrc, vecX vtmp) %{
...
instruct reduce_$2I_$1bits(iRegINoSp dst, iRegIorL2I isrc, ifelse($1, 64, vecD, vecX) vsrc, ifelse($1, 64, vecD, vecX) vtmp, rFlagsReg cr) %{
  predicate(n->in(2)->bottom_type()->is_vect()->element_basic_type() == T_BYTE ||
  ...
  format %{ "neon_$2_reduction $dst, $isrc, $vsrc\t# $2 reduction ($1 bits)" %}
  ins_encode %{
    __ neon_minmax_reduction_int(as_Register($dst$$reg), Matcher::vector_element_basic_type(this, $vsrc),
                                 as_Register($isrc$$reg), as_FloatRegister($vsrc$$reg),
                                 /* vector_length_in_bytes */ ifelse($1, 64, 8, 16), /* is_min */ ifelse($2, min, true, false),
                                 as_FloatRegister($vtmp$$reg));
  %}
  ins_pipe(pipe_slow);
%}')dnl
dnl              $1   $2
REDUCE_MAX_MIN_I(64,   min)
REDUCE_MAX_MIN_I(128,  min)
REDUCE_MAX_MIN_I(64,   max)
REDUCE_MAX_MIN_I(128,  max)
dnl
define(`REDUCE_MAX_MIN_L', `
instruct reduce_$1L_128bits(iRegLNoSp dst, iRegL isrc, vecX vsrc, rFlagsReg cr) %{
  ...
  format %{ "neon_minmax_reduction_long $dst, $isrc, $vsrc\t# $1L reduction (128 bits)"
  %}
  ins_encode %{
...
instruct insert$2_$3bits(ifelse($1, 8, vecD, vecX) dst, ifelse($1, 8, vecD, vecX) src, ifelse($2, I, iRegIorL2I, iRegL) val, immI idx)
%{
  ...
  format %{ "orr    $dst, T$1B, $src, $src\n\t"
            "mov    $dst, ifelse($2, I, B/H/S, D), $idx, $val\t# insert$2 into vector($3bits)" %}
  ins_encode %{
    if (as_FloatRegister($dst$$reg) != as_FloatRegister($src$$reg)) {
      __ orr(as_FloatRegister($dst$$reg), __ T$1B,
             as_FloatRegister($src$$reg), as_FloatRegister($src$$reg));
    }
    __ mov(as_FloatRegister($dst$$reg),
           __ ifelse($2, I, elemType_to_regVariant(Matcher::vector_element_basic_type(this)), D),
           $idx$$constant, $val$$Register);
  %}
  ins_pipe(pipe_slow);
%}')dnl
dnl             $1  $2 $3
VECTOR_INSERT_I(8,  I, 64)
VECTOR_INSERT_I(16, I, 128)
VECTOR_INSERT_I(16, L, 128)
dnl
