...
source %{
  ...
  bool masked_op_sve_supported(int opcode, int vlen, BasicType bt) {
    ...
    if ((opcode == Op_AddReductionVI || opcode == Op_AddReductionVL) ||
        ((opcode == Op_MinReductionV || opcode == Op_MaxReductionV) &&
         is_integral_type(bt))) {
...
define(`ADD_REDUCE_I_LT128', `
instruct reduce_addI_sve_$1bits`'(iRegINoSp dst, iRegIorL2I isrc, vReg vsrc, vReg vtmp) %{
  predicate(UseSVE > 0 && n->in(2)->bottom_type()->is_vect()->length_in_bytes() == ifelse($1, 64, 8, 16));
  match(Set dst (AddReductionVI isrc vsrc));
  ins_cost(3 * INSN_COST);
  effect(TEMP_DEF dst, TEMP vtmp);
  format %{ "neon_add_reduction_integral $dst,  $isrc,  $vsrc\t# add reduction $1 bits (sve)" %}
  ins_encode %{
...
// Generate lower cost NEON instructions instead.dnl
ADD_REDUCE_I_LT128(64)
ADD_REDUCE_I_LT128(128)

instruct reduce_addI_partial(iRegINoSp dst, iRegIorL2I src1, vReg src2, vReg vtmp,
                             pRegGov ptmp, rFlagsReg cr) %{
  predicate(UseSVE > 0 &&
            n->in(2)->bottom_type()->is_vect()->length_in_bytes() > 16 &&
            n->in(2)->bottom_type()->is_vect()->length_in_bytes() < MaxVectorSize);
...
// Generate lower cost NEON instructions instead.
...
instruct reduce_$1$2_masked_partial($5 dst, $5 src1, vReg src2, pRegGov pg,
  ...
  ins_pipe(pipe_slow);
%}')dnl
define(`REDUCE_MAXMIN_I_LT16', `
instruct reduce_$1I_sve_$2bits(iRegINoSp dst, iRegIorL2I isrc, vReg vsrc, vReg vtmp, rFlagsReg cr) %{
  predicate(UseSVE > 0 &&
            n->in(2)->bottom_type()->is_vect()->length_in_bytes() == ifelse($2, 64, 8, 16) &&
  ...
  ins_cost(4 * INSN_COST);
  format %{ "neon_$1_reduce_int $dst, $isrc, $vsrc\t# $1I reduction $2 bits (sve)" %}
  ins_encode %{
    __ neon_minmax_reduction_int(as_Register($dst$$reg), Matcher::vector_element_basic_type(this, $vsrc),
                                 as_Register($isrc$$reg), as_FloatRegister($vsrc$$reg),
                                 /* vector_length_in_bytes */ ifelse($2, 64, 8, 16), /* is_min */ ifelse($1, min, true, false),
                                 as_FloatRegister($vtmp$$reg));
  %}
  ins_pipe(pipe_slow);
%}')dnl
define(`REDUCE_MAXMIN_L_16', `
instruct reduce_$1L_sve_128bits(iRegLNoSp dst, iRegL isrc, vReg vsrc, rFlagsReg cr) %{
  predicate(UseSVE > 0 &&
            n->in(2)->bottom_type()->is_vect()->length_in_bytes() == 16 &&
            n->in(2)->bottom_type()->is_vect()->element_basic_type() == T_LONG);
  match(Set dst (ifelse($1, min, MinReductionV, MaxReductionV) isrc vsrc));
  ...
  format %{ "neon_minmax_reduction_long $dst, $isrc, $vsrc\t# $1L reduction 128 bits (sve)"
  %}
  ins_encode %{
    __ neon_minmax_reduction_long(as_Register($dst$$reg), as_Register($isrc$$reg),
                                  as_FloatRegister($vsrc$$reg), /* is_min */ ifelse($1, min, true, false));
  %}
  ins_pipe(pipe_slow);
%}')dnl
...
// Generate lower cost NEON instructions instead.dnl
REDUCE_MAXMIN_I_LT16(max, 64)
REDUCE_MAXMIN_I_LT16(max, 128)
REDUCE_MAXMIN_L_16(max)
...
REDUCE_MAXMIN_I_LT16(min, 64)
REDUCE_MAXMIN_I_LT16(min, 128)
REDUCE_MAXMIN_L_16(min)
...
instruct insertI_64or128bits(vReg dst, vReg src, iRegIorL2I val, immI idx) %{
  predicate(UseSVE > 0 &&
...
instruct insertL_sve_128bits(vReg dst, vReg src, iRegL val, immI idx) %{
...
instruct insertF_64or128bits(vReg dst, vReg src, vRegF val, immI idx) %{
...
instruct insertD_sve_128bits(vReg dst, vReg src, vRegD val, immI idx) %{
  predicate(UseSVE > 0 && Matcher::vector_length_in_bytes(n) == 16 &&
            n->bottom_type()->is_vect()->element_basic_type() == T_DOUBLE);
  match(Set dst (VectorInsert (Binary src val) idx));
  ins_cost(2 * INSN_COST);
